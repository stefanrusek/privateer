Feature: Secret value redaction
  ...
  Scenario: Secret data values are redacted by default
    ...
    Then the yaml frame contains "[redacted]"
  ...
  Scenario: Non-Secret resources are not redacted
    ...
    And the yaml frame does not contain "[redacted]"
  ...
  Scenario: Reveal secret values after confirmation
    ...
    Then the yaml frame contains "c2VjcmV0"
    And the yaml frame does not contain "[redacted]"

  Scenario: stringData field is also redacted
